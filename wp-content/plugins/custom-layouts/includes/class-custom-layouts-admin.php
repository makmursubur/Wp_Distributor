...
' );
	...
	public static function get_editor_settings() {
		...
		if ( current_theme_supports( 'editor-styles' ) ) {
			// First, check if the current theme is an FSE theme (and the required functions exist)
			// When FSE is disabled, get_block_editor_settings still exists but doesn't return what we we want
			if ( function_exists( 'gutenberg_is_fse_theme' ) && gutenberg_is_fse_theme() && function_exists( 'get_block_editor_settings' ) ) {
				$editor_settings = get_block_editor_settings( array(), null );
			}
			// Gutenberg 10.5 +.
			else if ( function_exists( 'gutenberg_get_default_block_editor_settings' ) ) {
				$editor_settings           = gutenberg_get_default_block_editor_settings( 'post-editor' );
				$editor_settings['styles'] = self::get_editor_styles();
			}
			// WP 5.8 +.
			elseif ( function_exists( 'get_default_block_editor_settings' ) ) {
				$editor_settings           = get_default_block_editor_settings( 'post-editor' );
				$editor_settings['styles'] = get_block_editor_theme_styles();
			}
		} else {
			// Non GB
